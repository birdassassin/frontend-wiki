import{_ as t,o,c as n,a2 as a}from"./chunks/framework.BWuWLRhz.js";const d=JSON.parse('{"title":"","description":"","frontmatter":{},"headers":[],"relativePath":"wiki/legacy/React系列/redux-logger源码分析.md","filePath":"wiki/legacy/React系列/redux-logger源码分析.md"}'),r={name:"wiki/legacy/React系列/redux-logger源码分析.md"};function s(p,e,c,g,i,l){return o(),n("div",null,[...e[0]||(e[0]=[a(`<p>在redux的配置文件中，如果你使用了redux-logger，也许你会写下面这样一段代码：</p><pre><code>import thunk from &#39;redux-thunk&#39;;
...
export default createLogger;
    ...
    if (typeof predicate === \`function\` &amp;&amp; !predicate(getState, action)) &amp;#123;
      return next(action);
    &amp;#125;
</code></pre><p><strong>6、给各个常量赋值</strong> 为什么会有这么多常量呢？我们来看一张图，图上展示了需要打印的各种信息。</p><p><img src="https://gitee.com/birdassassin/frontend-wiki/raw/master/img/bVQK0J" alt="clipboard.png"></p><p><strong>总结出来就是：</strong></p><p>action <strong>action.type</strong> @ <strong>timer</strong> prev state <strong>{}</strong> action <strong>{}</strong> next state <strong>{}</strong></p><p><strong>这里需要的是action.type, timer, 各种状态下的state</strong></p><pre><code>const started = timer.now();
const prevState = transformer(getState());
...
</code></pre><h4 id="总结" tabindex="-1">总结 <a class="header-anchor" href="#总结" aria-label="Permalink to &quot;总结&quot;">​</a></h4><p>redux-logger做的事情是在控制台输出action的信息，所以首先要获取前一个action，当前action，然后是下一个action。看完之后，你对redux-logger源码的理解加深了吗？</p><p>如果觉得文章对你有帮助，在你点击收藏的同时，点个赞。</p>`,50)])])}const u=t(r,[["render",s]]);export{d as __pageData,u as default};
