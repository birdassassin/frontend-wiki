...
<span class="line"><span>Prop 验证</span></span></code></pre></div><p>简单来说就是通过props实现父组件的数据流向子组件。</p><p><img src="`+n+`" alt="clipboard.png"></p><p>为什么叫做单向数据流呢？ 数据在父组件中可以修改，比如通过http请求动态更新数据，而子组件只负责通过props接收数据，子组件的权限是只读（如果我没理解错的话，那么跟react中是一样的。）</p><p>在本例子中，我实现了这样一个父组件Home.vue</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>&lt;template&gt;</span></span>
