import n from"/img/bVHksO";import a from"/img/bVHks8";import{_ as p,o as e,c as l,a2 as t}from"./chunks/framework.BWuWLRhz.js";const b=JSON.parse('{"title":"","description":"","frontmatter":{},"headers":[],"relativePath":"wiki/legacy/React系列/antd组件开发思路——alert分析.md","filePath":"wiki/legacy/React系列/antd组件开发思路——alert分析.md"}'),c={name:"wiki/legacy/React系列/antd组件开发思路——alert分析.md"};function i(o,s,r,d,m,f){return e(),l("div",null,[...s[0]||(s[0]=[t('<p>先附上antd源码地址：<a href="https://github.com/ant-design/ant-design/tree/master/components/alert" target="_blank" rel="noreferrer">https://github.com/ant-design/ant-design/tree/master/components/alert</a></p><p>昨天写了一篇分析antd之button组件的分析，今晚继续讲antd组件篇，这篇文章主要介绍的是alert实现原理，以及我们可以从antd的组件思想中学习到的react组件开发知识。 ps：antd用的是typescript，如果是纯ES写法稍微有些不同。</p><p>下面这张图是alert组件的主要结构图。 <img src="'+n+'" alt="alert"></p><p>有这么几个部分： 1、demo：alert组件的使用方法 2、style：组件内部可能用到的初始化样式 3、2个.md说明文档，一个是英文版，一个是中文版 4、index.tsx：alert组件（关于这个组件，我是有话要说的，这个命名应该用alert，然后index通常是用来导出alert组件，antd每个组件都不是同一个人写的，估计写alert组件的人也没考虑那么多。）</p><p>大概知道了alert项目文件的构成之后，如何去分析组件怎样实现的呢？ 先别看代码，看一下?提供的中文文档。 <img src="'+a+`" alt="图片描述"></p><p>主要看API部分，这些api就是组件内部需要定义的接口，一共有8个参数，包括类型、事件等可能需要用到的功能。假设你们公司也打算用react来封装自己的组件，首先要考虑的是制定这样一份API方案，确定需要实现的功能以及保留的功能。</p><p>看完文档之后，对alert组件的数据模型有了一个大概的了解，那么接下来就要看看代码是如何实现的。 react组件其实就是一个JSX语法组成的模板，给dom绑定事件，从外部传入需要的参数等。</p><p>下面这个是index.tsx的源码：</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>import React from &#39;react&#39;; //react都认识了</span></span>
